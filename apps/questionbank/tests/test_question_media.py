...
class QuestionMediaUnitTest(TestSetUp):
    ...
    # ?                  UNIT - TESTS
    # ?###################################################
    def do_create_question_media(self, request_body):
        print_test_header("create_question_media")
        url = "/api/question-media/"
    ...
    def do_delete_one_question_media(self, question_media_id):
        ...
        url = f"/api/question-media/{question_media_id}/"
        response = self.client.delete(url, headers=self.headers)
        validate_success_204_test_response(self, response)


class QuestionMediaTest(QuestionMediaUnitTest):

    # ?###################################################
    # ?              TESTS - CASES
    # ?###################################################
    def test_cases_question_media(self):
    ...
    def successfull_creation_of_a_record_test(self):
        ...
        json_data = self.do_create_question_media(request_body)
        validation_keys = [
            "id",
            "media",
            "description",
    ...
    def successfull_deletion_of_a_record_test(self, test_record_id):
        self.do_delete_one_question_media(test_record_id)
