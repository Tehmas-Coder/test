...
class CandidateSelfPreparationExamUnitTest(TestSetUp):
    fixtures = [
        "question_type_seed",
        "measuring_unit_seed",
        "subject_seed",
    ...
    def do_create_candidate_self_preparation_exam(self, request_body):
        print_test_header("create_candidate_self_preparation_exam")
        url = "/api/candidate-exam/candidate-self-preparatory-exam/"
...
class CandidateSelfPreparationExamTest(CandidateSelfPreparationExamUnitTest):
    ...
    def successfull_creation_of_a_record_test(self):
        self.custom_login(email="cyberaxescandidate@gmail.com", password="12345678")
        response_data = self.do_create_candidate_self_preparation_exam(json.dumps(self.request_body))
        for field in self.list_of_fields_of_candidate_exam_model:
            self.assertIn(
                field,
                response_data,
                f"Field '{field}' is missing in the response data",
            )
